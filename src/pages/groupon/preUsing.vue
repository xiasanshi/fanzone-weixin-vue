<template>
    <div>
        <div v-for="item in historyList">
            <div style="height: 90px;margin-bottom: 5px" v-if="item.orderStatus === 'SUCC'" @click="toJoin(item.teamId)" class="bg_ff">
                ...
                <div style="padding: 10px 0 0 10px;" class="position_re">
                    ...
                    <div style="font-size: 12px;color: #666666">
                        使用有效期至 {{item.useDurationEndTime}}
                    </div>
                    <div style="font-size: 12px;color: #666666">
                        验券码：{{item.orderCode}}
                    </div>
    ...
    export default {
        ...
        filters: {
            ...
            formatGrouponStatus: function (val) {
                return val == 'REFUNDSED' ? '拼团失败已退款' : val == 'GROUPING' ? '待成团' : val == 'PREPAY' ? '代付款' : val == 'preUSING' ? '待使用' : val == 'SUCC' ? '待使用' : ''
            }
        },
        mounted() {
            document.title = '待使用'
